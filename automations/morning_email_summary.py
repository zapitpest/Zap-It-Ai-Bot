"""
Morning Email Summary — runs at 7:00 AM daily.
Scans Gmail for all emails since 5:00 PM the previous day.
Flags real estate work orders at the top. Ignores hipages leads.
"""
...
def run() -> None:
    logger.info("Running morning email summary")
    ...
    lines = ["<h2>Morning Email Summary</h2>",
             f"<p>Emails received since 5:00 PM yesterday ({len(emails)} total, "
    ...
    send_email(
        to=BUSINESS_EMAIL,
        subject=f"Morning Summary — {datetime.now().strftime('%a %d %b %Y')}",
        content=content,
    )
    logger.info("Morning summary sent (%d emails processed)", len(emails))
